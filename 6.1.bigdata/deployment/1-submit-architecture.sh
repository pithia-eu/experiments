...
echo "Submitting bigdata.yml to MiCADO at $MICADO_MASTER with appid \"$APP_ID\"..."
curl --insecure -s -F adt=@"bigdata.yml" -X POST -u "$SSL_USER":"$SSL_PASS" https://$MICADO_MASTER:$MICADO_PORT/toscasubmitter/v2.0/applications/$APP_ID/ | jq
